...
public class Methods {
	...
	byte[] input= new byte[password.length()+salt.length];//password+salt길이를 가지는 배열 생성
	
	System.arraycopy(Utils.toByteArray(password), 0, input, 0, password.length());//input배열에 password를 붙여넣는다.
	System.arraycopy(salt, 0, input, password.length(), salt.length);//input배열에 salt를 붙여넣는다.
	...
	password_check_out=hash.digest(password_check_out);//hash함수에 넣고 결과를 얻어낸다.
